...
if pdf_file and st.button("1️⃣ 텍스트 추출 및 파싱"):
    ...
    question_imgs = extract_question_images(tmp.name)
    ...
    st.session_state.question_imgs = question_imgs
...
if "parsed_data" in st.session_state:
    ...
    if "question_imgs" in st.session_state:
        st.subheader("🖼 추출된 문제 이미지")
        import base64
        for img in st.session_state.question_imgs:
            st.image(base64.b64decode(img["image"]), caption=f"p{img['page']} {img.get('label','')}")
